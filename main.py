...
try:
    from local_secrets import WIFI_SSID, WIFI_PASS, HOME_LAT, HOME_LON
except ImportError:
...
def draw_status_screen(gps_data, battery_data, wifi_connected, engine_on):
    ...
    if gps_data.get("time"):
        time_str = gps_data["time"][11:19] if len(gps_data["time"]) > 19 else "??:??:??"
...
def update_sensors():
    ...
    global sensor_data, is_syncing, last_wifi_attempt
